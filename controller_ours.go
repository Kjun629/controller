...
func (r *InferenceServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	*/
	...
	fmt.Printf("Reconciling inference service apiVersion:%+v, isvc:%v\n", isvc.APIVersion, isvc.Name)
	...
	fmt.Printf("total reconcilers:%+v\n", reconcilers)
	for _, reconciler := range reconcilers {
		result, err := reconciler.Reconcile(isvc)
		fmt.Printf("reconciler.result:%+v, err:%+v\n", result, err)
		if err != nil {
	...
	fmt.Printf("reconcile.raw.deployment...\n")
	...
	fmt.Printf("reconcile.modelconfig...\n")
...
func (r *InferenceServiceReconciler) updateStatus(desiredService *v1beta1api.InferenceService, deploymentMode constants.DeploymentModeType) error {
	...
	fmt.Printf("updateStatus...., desired:%+v, deploymode:%+v\n", desiredService, deploymentMode)
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		r.Recorder.Eventf(desiredService, v1.EventTypeNormal, "testState", "testMSG")
		if inferenceServiceStatusEqual(existingService.Status, desiredService.Status, deploymentMode) {
			fmt.Printf("status equal\n")
			...
			fmt.Printf("was not Ready\n")
				...
				/*
				fmt.Println("++++++")
				fmt.Println("InferenceService is not Ready because of:")
				fmt.Println("......")
				fmt.Println(msg)
				fmt.Println("++++++")
				*/
			...
			fmt.Printf("wasready:%+v, isready:%+v\n", wasReady, isReady)
			if wasReady && !isReady { // Moved to NotReady State
				//1235467891123456789
				msg := GetFailConditions(desiredService)
				r.Recorder.Eventf(desiredService, v1.EventTypeWarning, string(InferenceServiceNotReadyState),
					fmt.Sprintf("InferenceService [%v] is no longer Ready because of: %v", desiredService.GetName(), msg))
			//12345678941236854687346446465
			/*
			fmt.Println("++++++")
			fmt.Println("InferenceService is no longer Ready because of: ")
			fmt.Println("......")
			fmt.Println(msg)
			fmt.Println("++++++")
			*/
...
func inferenceServiceReadiness(status v1beta1api.InferenceServiceStatus) bool {
	...
	logf.SetLogger(zap.New())
	log := logf.Log.WithName("my_controller")
	log.Info("inferenceServiceReadiness")
	fmt.Println("my_inferenceServiceReadiness")
	*/
...
func inferenceServiceStatusEqual(s1, s2 v1beta1api.InferenceServiceStatus, deploymentMode constants.DeploymentModeType) bool {
	...
	logf.SetLogger(zap.New())
	log := logf.Log.WithName("my_controller")
	log.Info("inferenceServiceStatusEqual")
	fmt.Println("my_inferenceServiceStatusEqual")
	*/
	if deploymentMode == constants.ModelMeshDeployment {
...
func (r *InferenceServiceReconciler) SetupWithManager(mgr ctrl.Manager, deployConfig *v1beta1api.DeployConfig, ingressConfig *v1beta1api.IngressConfig) error {
	fmt.Printf("SetupWithManager..\n")
	/*
	logf.SetLogger(zap.New())
	log := logf.Log.WithName("my_controller")
	log.Info("SetupWithManager")
	fmt.Println("my_SetupWithManager")
	*/
...
func (r *InferenceServiceReconciler) deleteExternalResources(isvc *v1beta1api.InferenceService) error {
	...
	logf.SetLogger(zap.New())
	log := logf.Log.WithName("my_controller")
	log.Info("deleteExternalResources")
	fmt.Println("my_deleteExternalResources")
	*/
